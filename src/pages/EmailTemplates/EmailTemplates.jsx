...
const EmailTemplates = () => {
  ...
  const { data: templateData, isLoading: isTemplateLoading } =
    useGetEmailByModuleQuery({ module: "Order", companyId: 2 });
  ...
  const { data: paramsData, isLoading: isParamsLoading } =
    useGetParametersQuery({ type: "Order" });
  ...
  // Prefill state with template data
    useEffect(() => {
      if (templateData?.data[0]) {
        ...
        const module = modules.find((m) => m.label === templateData?.data[0].ModuleType);
        if (module) {
          ...
          <div className="flex flex-wrap gap-2">
            {modules.map((item) => (
              <button
                ...
                `}
                onClick={() => setSelectedModule(item.value)}
              >
                {item.label}
              </button>
            ))}
