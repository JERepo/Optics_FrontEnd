...
export function POViewPage() {
  ...
  const updateQty = async () => {
    ...
    if (
      ...
      Number(newQty) > Number(selectedOrder.orderQty)
    ) {
          ...
          <td className="px-6 py-4 whitespace-wrap min-w-72">
            ...
            {order?.specs?.addOn?.addOnId && (
              <>
                ...
                <span className="font-medium">
                  AddOn: {order?.specs?.addOn?.addOnName}
                </span>
              </>
            )}
            {order?.specs?.tint?.tintCode && (
              <>
                <br />
                <span className="font-medium">
                  Tint: {order?.specs?.tint?.tintName}
                </span>
              </>
            )}
            {order?.hSN && (
    ...
    >
      ...
      <div key={poData.vendor?.Id || "vendor"} className="gap-12 my-10">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
          ...
          <p className="text-gray-700">
            ...
            <span className="flex">
              {poData.vendor?.Address1 || ""} {poData.vendor?.Address2 || ""}
            </span>
              ...
              {poreviewDetails.map((order, index) => (
                <tr key={order.id || index}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {order.orderPrefix}/{order.orderNo}/{order.slNo}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {order.productType === 0
                      ? "OL"
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {(() => {
                      const bothLens =
                        order?.specs?.powerDetails?.bothLens === 1;
                      const quantity =
                        order.poQty ??
                        order.orderQty - order.billedQty - order.cancelledQty;
                      const price =
                      ...
                      const taxPercentage =
                        parseFloat(order.taxPercentage) / 100 || 0;

                      const tintBuying =
                        parseFloat(order?.specs?.tint?.tintBuyingPrice || 0) ||
                        0;
                      ...
                      if (bothLens) {
                        ...
                        total =
                          price * quantity + tintBuying / 2 + addonBuying / 2;
                      }
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {order?.ProductDetails?.ProductType === 0 ? (
                      <td className="px-6 py-4 whitespace-wrap min-w-72">
                        ...
                        <br />
                        {order?.ProductDetails?.Specs?.Spherical
                          ? `Sph: ${order?.ProductDetails?.Specs?.Spherical} `
                          : `Sph: `}
                        {order?.ProductDetails?.Specs?.Cylinder
                          ? `Cyl: ${order?.ProductDetails?.Specs?.Cylinder} `
                          : `Cyl: `}
                        {order?.ProductDetails?.Specs?.Diameter
                          ? `Dia: ${order?.ProductDetails?.Specs?.Diameter} `
                          : `Dia: `}
                        <br />
                        {order?.ProductDetails?.HSN &&
                          `HSN: ${order?.ProductDetails?.HSN}`}
                      </td>
                    ) : order?.ProductDetails?.ProductType === 1 ? (
                      <td className="px-6 py-4 whitespace-wrap">
                        {order?.ProductDetails?.productName}
                      ...
                      <td className="px-6 py-4 whitespace-wrap">
                        {order?.ProductDetails?.productName}
                        <br />
                        {order?.ProductDetails?.PowerSpecs?.Sph
                          ? `Sph: ${order?.ProductDetails?.PowerSpecs?.Sph} `
                          : `Sph: `}
                        {order?.ProductDetails?.PowerSpecs?.Cyl
                          ? `Cyl: ${order?.ProductDetails?.PowerSpecs?.Cyl} `
                          : `Cyl: `}
                        {order?.ProductDetails?.PowerSpecs?.Axis
                          ? `Axis: ${order?.ProductDetails?.PowerSpecs?.Axis} `
                          : `Axis: `}
                        {order?.ProductDetails?.PowerSpecs?.Add
                          ? `Add: ${order?.ProductDetails?.PowerSpecs?.Add} `
                          : `Add: `}
                        <br />
                        {order?.ProductDetails?.HSN &&
                          `HSN: ${order?.ProductDetails?.HSN}`}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {order?.ProductDetails?.productName}
                        <br />
                        {order?.ProductDetails?.hsncode
                          ? `HSN: ${order?.ProductDetails?.hsncode}`
                          : null}
                      </td>
                    )}
                  </td>
            ...
            <div className="mb-4">
              ...
              {poData.againstOrder === 1 && selectedOrder.orderQty && (
                <p className="text-sm text-gray-600 mt-2">
                  Maximum allowed: {selectedOrder.orderQty}
                </p>
              )}
            </div>
            <div className="flex justify-end gap-4">
      ...
      <div className="flex mt-10 justify-between px-5 rounded-2xl shadow p-8">
        ...
        <div className="flex justify-between gap-4">
          ...
          <span className="font-bold text-lg">
            ₹ {Number(poData.totalGrossValue).toFixed(2) || "0.00"}
          </span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="text-gray-600 font-bold text-lg">Total GST:</span>
          <span className="font-bold text-lg">
            ₹ {Number(poData.totalGSTValue).toFixed(2) || "0.00"}
          </span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="text-gray-600 font-bold text-lg">
          ...
          <span className="font-bold text-lg">
            ₹ {Number(poData.totalValue).toFixed(2) || "0.00"}
          </span>
        </div>
      </div>
