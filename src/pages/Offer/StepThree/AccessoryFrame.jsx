...
const FrameSunglass = () => {
  ...
  const [saveAcc, { isLoading: isAccSaving }] = useSaveOfferProductMutation();
  ...
  const handleBarcodeSubmit = async (e) => {
    ...
    const res = await fetchByBarcode({
      barcode,
      locationId: customerOffer.locationId,
    });
  ...
  const frameBrands = useMemo(() => {
    ...
    return allBrandsData.filter(
      (b) => b.IsActive === 1 && b.OthersProductsActive === 1
    );
  }, [allBrandsData]);
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {isLoadingAllBrands ? (
                ...
                Object.entries(groupedBrands).map(([groupId, brands]) => (
                  <div
                    key={groupId}
                    className="border border-neutral-300 rounded-lg p-2 mb-4 shadow-sm"
                  >
                    <div className="flex gap-2">
                      {brands.map((brand) => (
                        <Checkbox
