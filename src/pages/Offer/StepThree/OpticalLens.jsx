...
const OpticalLens = () => {
  ...
  const generateProductName = (data) => {
    ...
    const addOn =
      ...
      "";
    const tint = data.tintvalue === 1 ? "Tint" : "";
            ...
            <div className="grid grid-cols-3 gap-5 mt-5">
              ...
              {lensData.brandId && lensData.type === 3 && (
                <AutocompleteField
                  label="Coating"
                  options={deduplicateOptions(
                    coatingDataByBrand?.data?.filter((b) => b.IsActive === 1) ||
                      [],
                    "Id"
                  )}
                  valueField="Id"
                ...
                <div className="w-1/3">
                  {addOnData?.data && (
                    <AutocompleteField
                      label="Add On"
                      options={deduplicateOptions(
                        addOnData?.data?.data.filter((b) => b.IsActive === 1) ||
                          [],
                        "Id"
                      )}
                      valueField="Id"
                      labelField="AddOnName"
                      value={lensData.AddOnId}
                      onChange={(val) =>
                        setLensData((prev) => ({ ...prev, AddOnId: val }))
                      }
                    />
                  )}
                </div>
