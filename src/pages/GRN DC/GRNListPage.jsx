...
export function GRNDCListPage() {
    ...
    const { data: grnResponse, isLoading: isAllGrnLoading } = useGetAllGRNDCMainQuery();
    ...
    const purchaseOrders = useMemo(() => {
        ...
        let filtered = [...allGrn].sort((a, b) => new Date(b.CreatedOn) - new Date(a.CreatedOn));
        ...
        if (searchQuery) {
            filtered = filtered.filter((grn) => {
                ...
                const grnPrefix = grn.GRNPrefix?.toLowerCase() || "";
                const vendorDocNo = grn.VendorDocNo?.toLowerCase() || "";
                const vendorMobile = grn.Vendor?.MobNumber?.toLowerCase() || "";

                return (
                    vendorName.includes(query) ||
                    grnNumber.includes(query) ||
                    grnPrefix.includes(query) ||
                    vendorDocNo.includes(query) ||
        ...
        <div className="max-w-8xl">
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <div className="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">GRN DC List</h1>
                        <p className="text-sm text-gray-500 mt-1">
                            Manage and track all your GRN DC orders in one place.
                        </p>
                    </div>
                </div>
                ...
                <div className="px-6 py-5">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                            <HasPermission module="GRN DC" action={["create"]}>
                                <button
                                    icon={FiPlus}
                                    className="flex gap-2 px-4 py-2 bg-blue-700 text-white rounded-lg hover:bg-primary transition-colors disabled:opacity-50"
                    ...
                    <Table
                        ...
                        renderRow={(grn, index) => (
                            <TableRow key={grn.id}>
                                ...
                                <TableCell>
                                    {grn.VendorDocDate ? new Intl.DateTimeFormat("en-GB", {
                                        day: "2-digit",
                                        month: "2-digit",
                                        year: "numeric",
                                    }).format(new Date(grn.VendorDocDate)) : "N/A"}
                                </TableCell>
