...
const VendorPaymentEntries = ({
  ...
  const [saveFinalPayment, { isLoading: isFinalSaving }] =
    useSaveVendorPaymentMutation();
  const { data: advanceData } = useGetAdvanceDataForVendorQuery({
  ...
  const handleSave = async () => {
    ...
    const finalStructure = {
      ...
      entries: items.map((item) => {
        return {
          GRNMainId: item.GRNMain?.Id ?? null,
          purchaseId: item?.purchaseMaster?.Id ?? null,
...
export default VendorPaymentEntries;
