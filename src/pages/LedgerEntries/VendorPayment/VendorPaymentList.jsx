...
import { useGetAllVPQuery } from "../../../api/vendorPayment";
...
const VendorPaymentList = () => {
  ...
  const { data: allCp, isLoading: isAllOrdersLoading } = useGetAllVPQuery();
  ...
  const customerPayments = useMemo(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter((invoice) => {
        ...
        const vendorName = invoice.Vendor?.VendorName?.toLowerCase() || "";
        const VendorMobile = invoice.Vendor?.MobNumber?.toLowerCase() || "";

        return vendorName.includes(query) || VendorMobile.includes(query);
      });
    }

    return filtered.map((c) => ({
      id: c.Id,
      date: format(c.PaymentDate, "dd/MM/yyyy"),
      customerName: c.Vendor?.VendorName,
      mobile: c?.Vendor?.MobNumber,
      amount: c.Amount,
    }));
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
              <Button
                ...
                onClick={() => {
                  goToStep(1);
                  navigate("/vendor-payment/create");
                }}
              >
                Add Vendor payment
              </Button>
